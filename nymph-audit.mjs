...
try{
 ...
 expect(result.instances===12,'all 12 nymphs were not replaced');expect(result.independent&&result.moved&&result.stopped,'independent walk/stop failed');expect(result.orientations.every(o=>o.upError<1e-6&&Math.abs(o.front[o.nx||o.ny?1:2]-(o.nx||o.ny?1:-1))<1e-6),'model forward/up does not match surface');
 ...
 const restart=await page.evaluate(()=>{const {hero,scene}=window.gameTest.models;return !window.gameTest.state.molting&&hero.scale.x===.75&&scene.children.filter(o=>o.userData.inner?.userData.nymph).length===12;});expect(restart,'restart retained a shed or growth');
