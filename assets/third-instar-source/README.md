...
ガクチョの反映指示により同一GLBを `public/models/roach-third-instar.glb` へ配置。`src/third-instar.ts` が材質・脚・触角単位で417メッシュを27描画メッシュへ結合し、独立した剛体グループを動かす。GLBそのものにスキンや骨を追加したものではない。第1章脱皮中の出現以降、第2章開始と脱皮後に使用し、既存の親スケールを維持。`third-instar-audit.mjs` は6脚/2触角・独立性・停止・白化/復色・再開始を検証する。詳細 `docs/september12-adjustments.md`。
