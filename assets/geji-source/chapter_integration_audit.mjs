...
await p.clock.install({time:new Date('2026-09-12T12:00:00Z')});await p.clock.pauseAt(new Date('2026-09-12T12:01:00Z'));await p.evaluate(()=>document.querySelector('#begin').click());
...
for(const [index,elapsed] of [[0,2.1],[1,7.1]]){
 await p.evaluate(({index,elapsed})=>{const t=window.chapter2Test,l=t.maze.gejis[index];while(t.snapshot().elapsed<elapsed)t.step(Math.min(.02,elapsed-t.snapshot().elapsed));t.setPosition((l.xLeft+l.xRight)/2,l.y+1.4);},{index,elapsed});
...
if(!report.loaded||!report.aligned||!report.onBoard||!report.visible||maxSkate>.02||walked<.5||errors.length||failed.length)process.exitCode=1;
