...
def box(name, loc, scale, material, bevel=.025, segments=1):
    ...
    b=o.modifiers.new('hard edge chamfer','BEVEL'); b.width=bevel; b.segments=segments; o.data.materials.append(material); return o
...
hx,hy,hz=centers[0]; box('head',(hx,hy+.19,hz+.015),(.285,.215,.14),HEAD,.065,3); box('head_shield',(hx,hy+.34,hz+.07),(.235,.12,.09),HEAD,.035,2)
...
# The lowest tarsus bound is z=.018756; the floor is deliberately coplanar.
bpy.ops.mesh.primitive_plane_add(size=200, location=(0,0,.018756)); bpy.context.object.name='contact_floor'; bpy.context.object.data.materials.append(mat('background',(.018,.020,.018),0,.72))
