...
try{
 ...
 await page.evaluate(()=>{window.chapter2Test.setPosition(7,40);window.chapter2Test.setEnemy('chase',36);});await page.waitForTimeout(180);let follow=await page.evaluate(()=>window.chapter2Test.snapshot());expect(follow.enemy.y>36.4,'centipede did not follow player');
 await page.evaluate(()=>window.chapter2Test.setPosition(3,38));expect((await page.evaluate(()=>window.chapter2Test.snapshot())).sheltered,'passable gap did not shelter player');
 ...
 await page.evaluate(()=>window.chapter2Test.setPosition(10.3,29));await page.mouse.down();await page.waitForTimeout(100);expect((await page.locator('#warning').textContent())?.includes('通れない'),'narrow gap feedback missing');await page.mouse.up();
 await page.evaluate(()=>{window.chapter2Test.setPosition(9,40);window.chapter2Test.setEnemy('chase',40,0,7);});const passing=await page.evaluate(()=>window.chapter2Test.snapshot());await page.waitForTimeout(70);expect((await page.evaluate(()=>window.chapter2Test.snapshot())).survivors===passing.survivors,'centipede hit without head contact');
 await page.evaluate(()=>{window.chapter2Test.setPosition(7,40);window.chapter2Test.setEnemy('warning',40,0,7);});const before=(await page.evaluate(()=>window.chapter2Test.snapshot())).survivors;await page.waitForTimeout(300);state=await page.evaluate(()=>window.chapter2Test.snapshot());expect(state.survivors===before&&state.enemy.state==='warning','centipede caught before 0.6s warning');await page.waitForTimeout(650);state=await page.evaluate(()=>window.chapter2Test.snapshot());expect(state.survivors===before-1,'centipede contact did not remove exactly one survivor');
 ...
 console.log('chapter2 audit PASS: Three.js scene, router, 8/13 survivors, gap shelter, narrow rejection, chase follow, head-only contact, 0.6s warning, one loss, page errors=0');
