...
document.querySelector('#again')!.addEventListener('click',()=>{moltVisual.reset();hero.userData.instar=1;game=new Game();hatchTime=0;hatchStarted=performance.now();remembered.clear();rebuildPickups();ended=false;document.querySelector<HTMLElement>('#ending')!.hidden=true;keys.clear();lastPopulation=game.siblings.length+1;lossNotice=0;lastX=game.player.x;lastY=game.player.y;lastZ=0;});
