...
scene.add(new THREE.HemisphereLight('#b1becb','#302322',2));
...
box(50,5.62,-2.6,100,.2,.3,wood);box(50,-.15,0,110,1.5,4,wood);box(50,5,-3.5,110,14,.5,wood);
for(let i=0;i<23;i++){box(i*5,10,-1,4.8,2,3,pink);box(i*5,4,-3, .18,11,.3,wood);}
for(const x of [18,42,68,88]){
 box(x+4,5.62,-2.5,8,.2,.3,wood);
 const curve=new THREE.CatmullRomCurve3([new THREE.Vector3(x,1,0),new THREE.Vector3(x+.1,3,0),new THREE.Vector3(x,6,0),new THREE.Vector3(x+.2,9,0)]);
 scene.add(new THREE.Mesh(new THREE.TubeGeometry(curve,28,.095,7,false),wireMat));
}
const solids=OBSTACLES.map(o=>box(o.x+o.w/2,o.y+o.h/2,.3,o.w,o.h,1.6,o.h>2?wood:pink));
const eggMaterial=new THREE.MeshStandardMaterial({color:'#65432b',roughness:.68});
function eggHalf(side:number){const g=new THREE.Group();const m=new THREE.Mesh(new THREE.SphereGeometry(1,24,16,side<0?Math.PI:0,Math.PI),eggMaterial);m.scale.set(.68,.33,.3);g.add(m);for(let j=0;j<9;j++){const seam=new THREE.Mesh(new THREE.TorusGeometry(.25,.013,5,12,Math.PI),eggMaterial);seam.position.x=-.48+j*.12;seam.rotation.y=Math.PI/2;g.add(seam);}g.position.set(4,1,1.1);scene.add(g);return g;}
...
for(let i=0;i<190;i++){const x=rand()*102,y=.63+rand()*.14;const m=box(x,y,rand()*3-1.5,.02+rand()*.15,.02+rand()*.06,.05,wood);m.rotation.z=rand()*3;}
const dustPositions=new Float32Array(450*3);for(let i=0;i<450;i++){dustPositions[i*3]=rand()*110;dustPositions[i*3+1]=rand()*12;dustPositions[i*3+2]=rand()*6;}
...
const hero=createAnimal('roach',.75);const kin=Array.from({length:game.siblings.length},()=>createAnimal('roach',.75));const predators=Array.from({length:2},()=>createAnimal('spider',2.3));const ants=Array.from({length:game.ants.length},()=>createAnimal('ant',.48));scene.add(hero,...kin,...predators,...ants);
const moltVisual=new MoltVisual(scene,hero);
...
function rebuildPickups(){pickups.forEach(m=>{scene.remove(m);m.geometry.dispose();});pickups.length=0;game.resources.forEach(r=>{const m=new THREE.Mesh(new THREE.SphereGeometry(.18,12,8),r.type==='water'?waterMat:foodMat);m.position.set(r.x,r.y,.15);m.scale.y=.65;scene.add(m);pickups.push(m);});}
rebuildPickups();
const checkpointModels=game.checkpoints.map((c,i)=>{const g=new THREE.Group();const mat=new THREE.MeshStandardMaterial({color:'#47321d',roughness:1});for(let j=0;j<9;j++){const m=new THREE.Mesh(new THREE.SphereGeometry(.09,7,5),mat);m.scale.set(1.5,.7,1);m.position.set(Math.cos(j*2.4)*.3,Math.sin(j*2.4)*.16,0);g.add(m);}g.position.set(c.x,c.y,1.35);scene.add(g);return g;});
...
function darkness(){
 ctx.clearRect(0,0,veil.width,veil.height);ctx.fillStyle='rgba(3,5,7,.985)';ctx.fillRect(0,0,veil.width,veil.height);
 const p=project(game.player.x,game.player.y),scale=innerHeight/11.5,radius=scale*.9*sense();
 ctx.globalCompositeOperation='destination-out';const near=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,radius);near.addColorStop(0,'rgba(0,0,0,1)');near.addColorStop(.55,'rgba(0,0,0,.92)');near.addColorStop(1,'transparent');ctx.fillStyle=near;ctx.fillRect(p.x-radius,p.y-radius,2*radius,2*radius);
 const angle=Math.atan2(aim.y-game.player.y,aim.x-game.player.x),range=(probe?4:2.4)*sense(),spread=probe?.87:.52;
 const g=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,range*scale);g.addColorStop(0,'rgba(0,0,0,.9)');g.addColorStop(.7,'rgba(0,0,0,.65)');g.addColorStop(1,'transparent');ctx.fillStyle=g;ctx.beginPath();ctx.moveTo(p.x,p.y);for(let i=0;i<=50;i++){const a=angle-spread+spread*2*i/50;const r=reach(game.player.x,game.player.y,Math.cos(a),Math.sin(a),range);const q=project(game.player.x+Math.cos(a)*r,game.player.y+Math.sin(a)*r);ctx.lineTo(q.x,q.y);}ctx.closePath();ctx.fill();ctx.globalCompositeOperation='source-over';
 OBSTACLES.forEach((o,i)=>{if([[o.x-.1,o.y],[o.x+o.w+.1,o.y],[o.x,o.y+o.h+.1],[o.x+o.w/2,o.y-.1]].some(([x,y])=>visible(x,y)))remembered.add(i);if(remembered.has(i)){const a=project(o.x,o.y+o.h),b=project(o.x+o.w,o.y);ctx.strokeStyle='rgba(157,164,149,.23)';ctx.lineWidth=1;ctx.strokeRect(a.x,a.y,b.x-a.x,b.y-a.y);}});
...
let previous=performance.now(),accumulator=0;function frame(now:number){requestAnimationFrame(frame);const dt=Math.min((now-previous)/1000,.25);previous=now;
 ...
 const populationNow=game.siblings.filter(s=>s.alive).length+(game.state==='lost'?0:1);if(populationNow<lastPopulation)lossNotice=3;lastPopulation=populationNow;lossNotice=Math.max(0,lossNotice-dt);
 warningLabel.textContent=started&&hatchTime<4?'卵鞘の裂け目から、小さな命が這い出す。':game.antAttack>0?`アリが群がっている — 離れないと捕食される ${Math.max(0,3-game.antAttack).toFixed(1)}秒`:game.water<.12?'水が尽きかけている — 水滴を探す':game.hunger<.12?'体が弱っている — 餌を探す':lossNotice>0?'白い帯がひとつ、消えた。':'';
 ...
 solids.forEach((m,i)=>{m.rotation.z=Math.sin(now*.04+i)*game.humanEvent*.002;});
 kin.forEach((m,i)=>{const s=game.siblings[i];m.visible=!!s&&s.alive&&visible(s.x,s.y);if(s){const kdx=s.x-m.position.x,kdy=s.y-m.position.y;const kh=Math.hypot(kdx,kdy)>.001?Math.atan2(kdy,kdx):m.rotation.z;m.position.set(s.x,s.y,1.35);if(hatchTime<4){m.position.set(4+Math.cos(i*2.4)*hatchTime*.6,1+Math.abs(Math.sin(i*2.4))*hatchTime*.5,1.35);m.visible=started&&hatchTime>i*.18;}animateAnimal(m,now/1000+i*.3,Math.hypot(kdx,kdy)/Math.max(dt,.001),kh);}});
 predators.forEach((m,i)=>{const s=game.spiders[i];m.visible=!!s&&visible(s.x,s.y);if(s){m.position.set(s.x,s.y,1.4);animateAnimal(m,now/1000,s.state==="warning"?0:1,s.angle);m.scale.y=m.userData.baseScale*(s.state==="warning"?.72:1);m.position.z=s.state==="attack"?1.7:1.4;}});
 ants.forEach((m,i)=>{const a=game.ants[i];m.visible=!!a&&visible(a.x,a.y);if(a){m.position.set(a.x,a.y,1.35);animateAnimal(m,now/1000+i,1,a.angle);}});pickups.forEach((m,i)=>m.visible=game.resources[i].amount>0);
  crumbs.forEach((m,i)=>{const a=game.ants[i];m.visible=!!a&&a.carrying&&a.target===null&&visible(a.x,a.y);if(a)m.position.set(a.x+Math.cos(a.angle)*.13,a.y+Math.sin(a.angle)*.13,1.44);});
 checkpointModels.forEach((m,i)=>{const c=game.checkpoints[i];m.visible=visible(c.x,c.y);});
 const positions=dustGeometry.getAttribute('position') as THREE.BufferAttribute;for(let i=0;i<positions.count;i++){positions.setY(i,(positions.getY(i)-dt*(.015+game.humanEvent*.5)+12)%12);}positions.needsUpdate=true;
 warm.position.x=p.x;warm.position.y=p.y+2;warm.intensity=game.moltProgress>0?32:12;
 ...
 if(started&&!ended&&game.state!=='playing'){ended=true;const end=document.querySelector<HTMLElement>('#ending')!;end.hidden=false;end.querySelector('h2')!.textContent=game.state==='won'?'まだ、生きている。':'闇が、静かになった。';end.querySelector('p')!.textContent=game.state==='won'?'はじめての脱皮を終えた。':'白い帯は、もう動かない。';}
