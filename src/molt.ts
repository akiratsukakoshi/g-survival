...
export class MoltVisual {
 ...
 constructor(private scene:THREE.Scene,private hero:THREE.Group){
 ...
 update(progress:number,ready:boolean,molting:boolean,x:number,y:number,heading:number,time:number){
  if(molting&&!this.active){
   this.current=createAnimal('roach',.75);this.current.position.set(x,y,1.39);this.current.rotation.z=heading;
   ...
   this.scene.add(this.current);this.shells.push(this.current);
  }
  this.active=molting;
  ...
  this.hero.position.x=x+Math.cos(heading)*extract*.65;
  this.hero.position.y=y+Math.sin(heading)*extract*.65;
  this.hero.position.z=1.43+Math.sin(Math.PI*progress)*.09;
  this.hero.scale.set(.75*(1+.3*extract),.75*(1+.3*extract)*(1-.1*Math.sin(progress*Math.PI)),.75);
