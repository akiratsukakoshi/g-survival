...
export async function mountChapterTwo(){
 ...
 const moltReady=()=>!molted&&molting<=0&&chAt(player.x,player.y)==='M';
 ...
 function render(now:number,speed:number){const sink=humanTimer>=0?clamp(humanTimer/1.2,0,1)*-1.52:0;heroSurface.position.set(player.x,-player.y,.72+heightAt(player.x,player.y)+sink);const slopeX=(heightAt(player.x+.05,player.y)-heightAt(player.x-.05,player.y))/.1,slopeY=(heightAt(player.x,player.y+.05)-heightAt(player.x,player.y-.05))/.1;heroSurface.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,1),new THREE.Vector3(-slopeX,slopeY,1).normalize());
  animateAnimal(hero,now/1000,speed,player.angle);hero.scale.setScalar(molting>0?bodySize+(1.2-bodySize)*(1-molting/3.2):bodySize);
  ...
  for(const [i,model] of gejiModels.entries()){const lane=crossings[i];model.visible=lane.phase>=.9&&lane.phase<4.8;animateGeji(model,now/1000,{x:lane.x,y:-lane.y,z:-.095,heading:0,active:model.visible},(x,y)=>blockedAt(x,-y,.025,'enemy')?null:-.095+heightAt(x,-y));}
