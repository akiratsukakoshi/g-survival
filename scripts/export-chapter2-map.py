...
rows = re.findall(r"'([#.~12oMSGcgyv]+)'", source.split('export const MAP:string[]=[')[1].split('];')[0])
...
X, Y, S = 92, 204, 32
colors = {'v':'#b4dce9','#':'#596467', '.':'#fffdf7', '~':'#dac08a', '1':'#f6cf88', '2':'#b0dbcf', 'o':'#b4dce9', 'M':'#f4e7ad', 'S':'#227e67', 'G':'#e7b640', 'c':'#f1b5ac', 'g':'#ead0ad', 'y':'#d4bcdf'}
for c in range(14):
...
for r, row in enumerate(rows):
    ...
    for c, ch in enumerate(row):
        ...
        if ch=='v':d.rectangle((x,y,x+S,y+S*.375),fill=colors['#'])
        if ch in '12':
...
for addresses,color,label in [(['B18','I18','J28'],'#257cb1','水'),(['D16','H14','B8','L14'],'#a46d20','餌')]:
    for a in addresses:
        x=X+(ord(a[0])-65)*S;y=Y+(int(a[1:])-1)*S
        d.ellipse((x+3,y+3,x+29,y+29),fill=color);text(x+5,y+2,label,20,'white')
text(592, 196, '凡例', 28)
legend = [('#','壁・間柱（通行不可）'),('.','通常の通路'),('~','登れる断熱材の山'),('1','細い隙間：幅 1.10'),('2','広い隙間：幅 1.40'),('o','退避ポケット'),('v','下側に開く半マス退避穴'),('S','スタート'),('G','台所へ抜けるゴールの穴'),('c','赤線①〜③：ムカデ巡回'),('g','ゲジ往復の基準セル'),('y','ヤモリの巣')]
...
notes = [
 ('成長と退避', ['餌と水を蓄え → Space長押しで脱皮。','身体幅 1.02 → 1.20、速度 +12%（暫定）。','脱皮後は「1」不可、「2」は通れます。','隙間・ポケットには敵が入れません。']),
 ('位置の指定', ['H02：スタート ／ L45：ゴール','ムカデ3体：赤線を通路沿いに往復。','D22〜J22・D26〜J26：ゲジ2体','K33・D36：ヤモリ2体']),
 ('図の読み方', ['29〜40行・B〜M列を一つの部屋に。','ヤモリの部屋の断熱材は撤去。','D27/D28：細い隙間を連続配置。','J27/J28：広い隙間を連続配置。','赤線は巡回路（追跡範囲は含まない）。']),
...
text(60,1744,'ガクチョ指定の地形・敵追加を反映。速度・穴の深さ・ヤモリ余白はAI暫定。',19,'#657575')
