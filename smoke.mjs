...
test('eight directions, probe half speed, wall 30% bonus, solids',()=>{
 const a=isolated(),b=isolated();a.player={x:4,y:3,angle:0,z:0,nx:0,ny:0};b.player={x:4,y:3,angle:0,z:0,nx:0,ny:0};
 ...
 const c=isolated(),d=isolated();c.player={x:4,y:1,angle:0,z:0,nx:0,ny:0};d.player={x:4,y:3,angle:0,z:0,nx:0,ny:0};
 step(c,.3,{...idle,x:1});step(d,.3,{...idle,x:1});assert(Math.abs((c.player.x-4)/(d.player.x-4)-1.3)<.01);
 c.player={x:6.5,y:4,angle:0,z:0,nx:0,ny:0};step(c,.3,{...idle,x:1});assert(c.player.x<7);assert.equal(c.player.z,0); // 塊は塞ぐ。押し続けた先(登る)は climb の項で検証する
});
...
test('sprinting breaks ant contact before capture',()=>{
 const g=new Game();g.spiders=[];g.siblings.forEach(s=>s.alive=false);g.player={x:4,y:8,angle:0,z:0,nx:0,ny:0};g.hunger=g.water=1;
...
test('freeze hides, dash vibration beyond visual cone, occlusion',()=>{
 ...
 g.player={x:11.5,y:13.2,angle:0,z:0,nx:0,ny:0};assert.equal(g.prey(s,{...idle,freeze:true}),undefined);
 assert.equal(g.prey(s,{...idle,x:1,sprint:true}),g.player);
 g.player={x:20,y:20,angle:0,z:0,nx:0,ny:0};assert.equal(g.prey(s,{...idle,x:1,sprint:true}),undefined);
});
...
test('three checkpoints heal and gather, ants remain outside solids',()=>{
 const g=new Game();g.player={x:13,y:7,angle:0,z:0,nx:0,ny:0};g.health=.5;g.hunger=.5;step(g,1);assert(g.health>.59);assert(g.hunger>.5);assert.equal(g.checkpoints.length,3);
 const h=new Game();h.player={x:4,y:4,angle:0,z:0,nx:0,ny:0};h.siblings.forEach(s=>s.alive=false);
...
test('climb: 0.35 秒の押し込みで張り付き、天面に登り、踏み外して降り、高所はアリが届かない',()=>{
 const push={...idle,x:1};
 const g=isolated();g.player={x:6.75,y:4,angle:0,z:0,nx:0,ny:0};   // 塊 {x:7,y:2.5,w:4,h:3,top:.9} の西面
 step(g,.3,push);assert.equal(g.player.nx,0);assert.equal(g.player.z,0);   // 予告の 0.35 秒より前は貼り付かない
 step(g,.25,push);assert.equal(g.player.nx,-1);assert(Math.abs(g.player.x-6.84)<1e-9);assert(g.player.z>0);
 step(g,1,push);assert.equal(g.player.nx,0);assert(Math.abs(g.player.z-.9)<1e-9);assert(g.player.x>7&&g.player.x<11);
 step(g,3,push);assert.equal(g.player.z,0);assert(g.player.x>11.18);       // 東へ踏み外すと床まで降りる
 const a=new Game();a.spiders=[];a.siblings.forEach(s=>s.alive=false);
 Object.assign(a.player,{x:9,y:4,z:.9,nx:0,ny:0});a.ants.forEach(t=>{t.x=9;t.y=4;});
 step(a,1.5);assert.equal(a.antAttack,0);assert(!a.ants.some(t=>t.target===-1));
 a.player.z=0;step(a,1);assert(a.antAttack>0);                             // 床に降りれば従来どおり群がられる
 const b=isolated();b.hunger=b.water=1;b.player={x:6.75,y:4,angle:0,z:0,nx:0,ny:0};
 step(b,.5,push);assert.equal(b.player.nx,-1);
 step(b,3.2,{...idle,freeze:true});assert(!b.molting);                     // 壁に貼り付いたままは脱皮できない
