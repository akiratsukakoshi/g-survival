...
const idle={x:0,y:0,sprint:false,freeze:false,probe:false,aimX:100,aimY:1};
...
test('eight directions, probe half speed, wall 30% bonus, solids',()=>{
 ...
 c.player={x:8.5,y:4,angle:0};step(c,2,{...idle,x:1});assert(c.player.x<8.83);
});
...
test('ants contact, swarm, exact 3 seconds including molt, random handoff',()=>{
 const g=new Game();g.spiders=[];g.player={x:14,y:1,angle:0};g.hunger=g.water=1;g.molting=true;
 g.siblings.forEach((s,n)=>{s.x=4+n*.1;s.y=1;});g.ants.forEach(a=>{a.x=14;a.y=1;});
...
test('NPC ant swarm requires continuous contact and kills after 3 seconds',()=>{
 ...
 const s=g.siblings[0];Object.assign(s,{x:20,y:6.3,alive:true});g.ants.forEach(a=>{a.x=s.x;a.y=s.y;});
...
test('sprinting breaks ant contact before capture',()=>{
 const g=new Game();g.spiders=[];g.siblings.forEach(s=>s.alive=false);g.player={x:4,y:1,angle:0};g.hunger=g.water=1;
 g.ants.forEach(a=>{a.x=4;a.y=1;});step(g,.6);assert(g.antAttack>.5);
...
test('spider locks warning for .8 seconds, one capture per lunge',()=>{
 const g=new Game();g.ants=[];g.siblings.forEach(s=>s.alive=false);g.player={x:32,y:2.5,angle:0};
 g.spidersUpdate(1/60,{...idle,x:1});const s=g.spiders[0];assert.equal(s.state,'warning');const lock=s.targetY;
 g.player.y=2.8;for(let n=0;n<40;n++)g.spidersUpdate(1/60,idle);assert.equal(s.state,'warning');assert.equal(s.targetY,lock);
 const h=new Game();h.player={x:4,y:1,angle:0};h.spiders[0].state='attack';h.spiders[0].angle=0;h.spiders[0].x=30;
 h.siblings.forEach(q=>{q.x=30.3;q.y=2.5;q.alive=true;});h.spidersUpdate(1/60,idle);
...
test('freeze hides, dash vibration beyond visual cone, occlusion',()=>{
 const g=new Game();g.siblings.forEach(s=>s.alive=false);const s=g.spiders[0];s.x=31;s.y=2.5;s.angle=0;
 g.player={x:29,y:2.5,angle:0};assert.equal(g.prey(s,{...idle,freeze:true}),undefined);
 assert.equal(g.prey(s,{...idle,x:1,sprint:true}),g.player);
 g.player={x:24,y:6.3,angle:0};assert.equal(g.prey(s,{...idle,x:1,sprint:true}),undefined);
});
test('depletion is lethal, early resources cannot unlock molt, human events once',()=>{
 ...
 const h=isolated();for(const r of h.resources.filter(r=>r.x<75)){h.player.x=r.x;h.player.y=r.y;step(h,4);}
 assert(!h.moltReady);assert(h.water<.85);
 h.humanEventsTriggered=0;h.player.x=40;step(h,.1);assert(h.humanEvent>0);step(h,2);assert.equal(h.humanEvent,0);
 h.player.x=39;step(h,.1);h.player.x=40;step(h,.1);assert.equal(h.humanEvent,0);
 h.player.x=75;step(h,.1);assert(h.humanEvent>0);assert.equal(h.humanEventsTriggered,3);
});
test('three checkpoints heal and gather, ants remain outside solids',()=>{
 const g=new Game();g.player={x:14,y:1,angle:0};g.health=.5;g.hunger=.5;step(g,1);assert(g.health>.59);assert(g.hunger>.5);assert.equal(g.checkpoints.length,3);
 const h=new Game();h.player={x:4,y:1,angle:0};h.siblings.forEach(s=>s.alive=false);
