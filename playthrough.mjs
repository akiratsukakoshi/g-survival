...
const h=.25,nx=133,ny=133;
const free=(x,y)=>x>=1&&x<=34&&y>=1&&y<=34&&!OBSTACLES.some(o=>x>o.x-.23&&x<o.x+o.w+.23&&y>o.y-.23&&y<o.y+o.h+.23);
function route(a,b){const cell=p=>[Math.round((p.x-1)/h),Math.round((p.y-1)/h)];let [ax,ay]=cell(a),[bx,by]=cell(b);const key=(x,y)=>y*nx+x;const start=key(ax,ay),end=key(bx,by),queue=[start],prev=new Map([[start,-1]]);for(let head=0;head<queue.length;head++){const k=queue[head];if(k===end)break;const x=k%nx,y=Math.floor(k/nx);for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){const xx=x+dx,yy=y+dy,kk=key(xx,yy);if(xx<0||xx>=nx||yy<0||yy>=ny||prev.has(kk)||!free(1+xx*h,1+yy*h))continue;prev.set(kk,k);queue.push(kk);}}if(!prev.has(end))throw new Error(`No traversable route ${JSON.stringify(a)} -> ${JSON.stringify(b)}`);const points=[];for(let k=end;k!==start;k=prev.get(k))points.push({x:1+(k%nx)*h,y:1+Math.floor(k/nx)*h});return points.reverse();}
const game=new Game();const controls={x:0,y:0,sprint:false,freeze:false,probe:process.argv.includes('--probe'),aimX:34,aimY:34};
